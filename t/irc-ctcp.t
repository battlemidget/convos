...
t::Helper->irc_server_messages(
  qr{NICK}    => ['welcome.irc'],
  qr{USER}    => ":other_client!u2\@example.com PRIVMSG mojo_irc :\x{1}PING 1393007660\x{1}\r\n",
  $connection => '_irc_event_ctcp_ping',
  qr{:\x01PING \d+\x01} => ":other_client!u2\@example.com PRIVMSG mojo_irc :\x{1}TIME\x{1}\r\n",
  $connection           => '_irc_event_ctcp_time',
  qr{:\x01TIME\s}       => ":other_client!u2\@example.com PRIVMSG mojo_irc :\x{1}VERSION\x{1}\r\n",
  $connection           => '_irc_event_ctcp_version',
  qr{:\x01VERSION Convos \d+\.\d+\x01} =>
    ":other_client!u2\@example.com PRIVMSG superman :\x{1}ACTION msg1\x{1}\r\n",
