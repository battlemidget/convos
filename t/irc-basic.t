...
my $err;
$connection->send_p('', 'whatever')->catch(sub { $err = shift })->wait;
like $err, qr{without target}, 'send: without target';

$connection->send_p('#test_convos' => '0')->catch(sub { $err = shift })->wait;
...
$user->remove_connection_p($connection->id)->wait;
