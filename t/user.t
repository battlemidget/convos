...
note 'users order';
$main::time++;
$core->user({email => 'aaa@bbb.com'})->save_p->wait;
$core->user({email => 'bbb@bbb.com', registered => '1983-02-24T01:23:00Z'})->save_p->wait;
$core->user({email => 'ccc@bbb.com'})->save_p->wait;

my $users;
$core->backend->users_p->then(sub { $users = shift })->wait;
is_deeply(
  [map { $_->{email} } @$users],
